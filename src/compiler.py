...
def rco(prog: Program) -> Program:
    ...
    def rco_stmt(stmt: Stmt, new_stmts: List[Stmt]) -> Stmt:
        match stmt:
            ...
            case ClassDef(name, superclass, body):
                body_funcs = [stmt for stmt in body if isinstance(stmt, FunctionDef)]
                return ClassDef(name, superclass, rco_stmts(body_funcs))
            case FunctionDef(name, params, body_stmts, return_type):
            ...
            case If(condition, then_stmts, else_stmts):
                ...
                return If(new_condition, new_then_stmts, new_else_stmts)
            case While(condition, body_stmts):
                condition_stmts = []
    ...
    def cc_stmt(stmt):
        match stmt:
            ...
            case While(Begin(condition_stmts, condition_exp), body_stmts):
                ...
                return [
                    While(Begin(new_condition_stmts, new_condition_exp), new_body_stmts)
                ]
                ...
                match s:
                    case FunctionDef(name, params, body_stmts, return_type):
                        ...
                        function_defs.append(
                            cif.CFunctionDef(name, param_names, blocks)
                        )
                    case _:
    ...
    def explicate_stmt(stmt: Stmt, label: str) -> str:
        match stmt:
            ...
            case If(condition, then_stmts, else_stmts):
                ...
                if_stmt = cif.If(
                    explicate_exp(condition), cif.Goto(then_label), cif.Goto(else_label)
                )
                    ...
                    case cif.CFunctionDef(name, args, blocks):
                        ...
                        match p:
                            case x86.X86Program(new_blocks):
                                setup_instrs = [
                                    x86.Movq(x86.Reg(r), x86.Var(a))
                                    for a, r in zip(args, constants.argument_registers)
                                ]
                                new_blocks[name + "start"] = (
                                    setup_instrs + new_blocks[name + "start"]
                                )

                                function_defs.append(
                                    X86FunctionDef(name, new_blocks, None)
                                )
...
def _select_instructions(current_function: str, prog: cif.CProgram) -> x86.X86Program:
    ...
    def si_stmt(stmt: cif.Stmt) -> List[x86.Instr]:
        match stmt:
            ...
            case cif.Assign(x, cif.Prim("tuple", args)):
                ...
                instrs = [
                    x86.Movq(x86.Immediate(8 * (1 + len(args))), x86.Reg("rdi")),
                    x86.Callq("allocate"),
            ...
            case cif.Assign(x, cif.Prim("subscript", [atm1, cif.Constant(idx)])):
                ...
                return [
                    x86.Movq(si_expr(atm1), x86.Reg("r11")),
            ...
            case cif.If(a, cif.Goto(then_label), cif.Goto(else_label)):
                return [
                    x86.Cmpq(si_expr(a), x86.Immediate(1)),
    ...
    match prog:
        case cif.CProgram(basic_blocks):
            basic_blocks = {
                label: si_stmts(block) for (label, block) in basic_blocks.items()
            }
    ...
    match program:
        case X86ProgramDefs(defs):
            ...
            for d in defs:
                ...
                new_defs.append(
                    X86FunctionDef(d.label, new_program.blocks, new_program.stack_space)
                )
            return X86ProgramDefs(new_defs)


def _allocate_registers(
    current_function: str, program: x86.X86Program
) -> x86.X86Program:
    """
    Assigns homes to variables in the input program. Allocates registers and
    stack locations as needed, based on a graph-coloring register allocation
    ...
    # --------------------------------------------------
    def color_graph(
        local_vars: Set[x86.Var], interference_graph: InterferenceGraph
    ) -> Coloring:
        coloring: Coloring = {}
    ...
    return x86.X86Program(
        new_blocks, stack_space=(regular_stack_space, root_stack_slots)
    )
...
def _prelude_and_conclusion(
    ...
    # Prelude
    prelude = [x86.Pushq(x86.Reg("rbp")), x86.Movq(x86.Reg("rsp"), x86.Reg("rbp"))]
    ...
    for _ in range(root_stack_locations):
        prelude += [
            x86.Movq(x86.Immediate(0), x86.Deref("r15", 0)),
    ...
    # Conclusion
    conclusion = [
        x86.Addq(x86.Immediate(stack_bytes), x86.Reg("rsp")),
    ...
    conclusion += [x86.Popq(x86.Reg("rbp")), x86.Retq()]
